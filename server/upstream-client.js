...
export async function upstream(site, path, body, method = body ? 'POST' : 'GET') {
  ...
  if (!response.ok || payload?.success === false || (payload?.code != null && payload.code !== 0)) {
    ...
    else if (response.status === 404) message = loginRequest ? '上游不支持用户登录接口，请检查地址及版本。' : refreshRequest ? '上游不支持登录续期接口，请重新授权。' : keysRequest ? '上游不支持用户 API 密钥列表接口，请检查地址及版本。' : accountRequest ? '上游不支持账户信息接口，请检查地址及版本。' : '上游站点不支持所需的分组接口，请检查地址及版本。'
    ...
    const error = new SyncError(message, response.status === 403 ? 403 : response.status === 401 ? 401 : 502)
